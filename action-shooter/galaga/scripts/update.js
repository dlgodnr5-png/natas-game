...
function mobileSupport(fighter, torpedos, stats, sound) {
    ...
    gameEl.style.height = "130vw";
    gameEl.style.width = "98vw";
    gameEl.style.margin = "4vh 0 4vh 0";
    gameEl.style.touchAction = "none";

    // 하단 슬라이더/버튼은 숨김 — 손가락으로 게임 화면을 직접 드래그해 비행기 이동 + 터치로 발사
    var mc = document.getElementById("mobile-controls");
    if (mc) mc.style.display = "none";

    var canvas = document.getElementById('id-canvas');
    canvas.style.touchAction = "none";
    var HALF = fighter.size.width / 2;
    var active = false, fireTimer = null;

    function toCanvasX(clientX) {
        var r = canvas.getBoundingClientRect();
        var x = (clientX - r.left) / r.width * canvas.width;
        return Math.max(HALF, Math.min(canvas.width - HALF, x));
    }
    function shoot() { if (stats.currentTime > 0) fireTorpedo(fighter, torpedos, stats, sound); }

    canvas.addEventListener("touchstart", function(e) {
        e.preventDefault();
        active = true;
        if (e.touches[0]) fighter.center.x = toCanvasX(e.touches[0].clientX);
        shoot();
        if (!fireTimer) fireTimer = setInterval(function() { if (active) shoot(); }, 170); // 누르고 있으면 자동 연사
    }, { passive: false });

    canvas.addEventListener("touchmove", function(e) {
        e.preventDefault();
        if (e.touches[0]) fighter.center.x = toCanvasX(e.touches[0].clientX); // 손가락 따라 비행기 이동
    }, { passive: false });
